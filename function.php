...
{ //オリジナル設定
  ...
  try {
    ...
    // SQL文作成
    debug('msgが初回かどうか判定');
    $sql = 'SELECT COUNT(*) FROM bord as b INNER JOIN message as m ON b.id = m.bord_id WHERE b.id = :id ';
    $data = array(':id' => $id);
    $stmt = queryPost($dbh, $sql, $data);
    $msgFlg = $stmt->fetch(PDO::FETCH_ASSOC);
    debug('やり取りしたメッセージ数：' . print_r($msgFlg, true));

    debug('msgの取得開始');

    $sql = 'SELECT m.id AS m_id, product_id, bord_id, send_date, to_user, from_user, sale_user, buy_user, msg, b.create_date FROM message AS m RIGHT JOIN bord AS b';
    if (array_shift($msgFlg) == 0) {
      $sql .= ' ON b.id = m.bord_id WHERE b.id = :id ORDER BY send_date ASC';
    } else {
      $sql .= ' ON b.id = m.bord_id WHERE b.id = :id AND m.delete_flg = 0 ORDER BY send_date ASC';
    }
...
{
  if (!empty($_GET)) { //GETが空じゃなかったら
    $str = '?'; //$strに?を入れる
    foreach ($_GET as $key => $val) {
      if (!in_array($key, $arr_del_key, true)) { //GETの中に取り除きたいパラメータが入ってなければGET内のキー名と値をurlにくっつける //in-array配列に指定した値が存在する
        $str .= $key . '=' . $val . '&';
      }
    }
